...
class WeeklyForecast extends React.Component {
                    ...
                    <div className='dashboard-body-wrapper'>
                        <div className='dashboard-body'>
                            ...
                            </div>
                            {
                                ...
                                :this.state.dataRetrieved && this.state.startWeekDate != "" ?
                                    <div>
                                        <div className="dashboard-sub-heading-text" style={{textAlign: "left", paddingLeft: "20px"}}> Weekly Forecast - Next 12 weeks starting from {this.state.startWeekDate} </div>
    ...
    componentDidMount() {
        ...
        if(isLogged == "true"){
            ...
            var publicURL = ""
            var privateURL = "http://localhost:4000/forecastData/weeklyForecast"
                ...
                result.json().then((resp) => {
                    ...
                    for(var i = 0; i < resp.length; i++){
                        ...
                        for(var j = 0; j < forecast_data.length; j++){
                            ...
                            this.state.rows.push(<WeeklyChart item_name={currentItem.replace(/_/g, ' ')}  week={singleForecastData["week_category"]} data={singleForecastData["weekly_data"]}/>)

                            if(j == 0){
                                this.setState({
                                    startWeekDate: singleForecastData["weekly_data"][0]["date"]
                                })
                            }
                        }
                    }
